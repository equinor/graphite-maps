...
def test_single_response_returns_sparse_representation():
    ...
    j = 0
    result = _fit_single_response_boost(
        j, U_scaled, y_j, learning_rate=0.5, effective_dimension=None
    )

    assert len(result) == 3, (
        f"Expected 3-tuple (j, nonzero_indices, nonzero_values), got {len(result)}-tuple"
    )
    col_idx, nonzero_indices, nonzero_values = result
    assert col_idx == j
    assert isinstance(nonzero_indices, np.ndarray)
    assert isinstance(nonzero_values, np.ndarray)
    assert len(nonzero_indices) == len(nonzero_values)
